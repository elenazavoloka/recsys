...
def calc_per_type_similarity(X,Y,cont_var, cat_var,list_var):
  ...
  for i in range(len(cont_var)):
        diff[[cont_var[i]]]=float(X.iloc[cont_var[i]])-float(Y.iloc[cont_var[i]])
    # compare categ fields
  for j in range(len(cat_var)):
        if X.iloc[cat_var[j]]==Y.iloc[cat_var[j]]:
            diff[[cat_var[j]]]=1
        else:
            diff[[cat_var[j]]]=0    
    # compare cont fileds         
  for k in range(len(list_var)):
        if isinstance(X.iloc[list_var[k]], basestring) and isinstance(Y.iloc[list_var[k]], basestring):
            list_X=X.iloc[list_var[k]].split(',')
            list_Y=Y.iloc[list_var[k]].split(',')
